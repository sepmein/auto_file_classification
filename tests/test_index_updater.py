...
class TestIndexUpdater:
    ...
    def test_update_indexes_success(self, mock_chroma_client):
        """测试成功的索引更新"""
        # Mock ChromaDB
        mock_collection = Mock()
        mock_chroma_client.return_value.get_collection.return_value = mock_collection

        # Set the collection in the index updater
        self.index_updater.set_collection(mock_collection)
        ...
        print(f"IndexUpdater result: {result}")
        ...
        # llama_update is disabled in tests, so it's expected to fail
        assert results["llama_update"]["success"] is False
        assert results["llama_update"]["reason"] == "disabled"
    ...
    def test_update_indexes_file_not_moved(self):
        ...
        document_data = {"text_content": "测试内容", "embedding": [0.1, 0.2, 0.3]}
    ...
    def test_log_audit_record(self):
        ...
        with sqlite3.connect(self.config["database"]["sqlite_path"]) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM file_operations WHERE id = ?", (operation_id,)
            )
            row = cursor.fetchone()
    ...
    def test_update_file_status(self):
        ...
        result = self.index_updater._update_file_status(
        ...
        with sqlite3.connect(self.config["database"]["sqlite_path"]) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM file_status WHERE file_path = ?", (str(test_file),)
            )
            row = cursor.fetchone()
