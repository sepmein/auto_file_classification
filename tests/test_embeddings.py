...
from ods.embeddings.models import (
    LocalEmbeddingModel,
    APIEmbeddingModel,
...
class TestTextProcessor:
    ...
    def setup_method(self):
        """测试前准备"""
        self.config = {"max_chunk_size": 100, "overlap_size": 20, "min_chunk_size": 50}
...
class TestEmbeddingModels:
    ...
    def test_local_model_config(self):
        """测试本地模型配置"""
        config = {"model_name": "BAAI/bge-m3", "dimension": 1024, "max_length": 8192}
...
class TestEmbedder:
    ...
    def test_process_document(self, mock_model_class):
        ...
        mock_model.get_dimension.return_value = 1024
        mock_model.get_model_info.return_value = {"type": "local", "status": "loaded"}
        mock_model_class.return_value = mock_model
    ...
    def test_smart_chunking(self, mock_model_class):
        ...
        # 检查块长度不超过配置的最大长度
        assert all(
            len(chunk) <= embedder.text_processor.max_chunk_size for chunk in result
        )
