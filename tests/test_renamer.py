...
class TestRenamer:
    ...
    def test_init(self):
        """测试初始化"""
        assert self.renamer.default_template == '{{category}}-{{title}}-{{date}}.{{ext}}'
    ...
    def test_generate_filename_success(self):
        ...
        result = self.renamer.generate_filename(path_plan, document_data, classification_result)
    ...
    def test_generate_filename_error(self):
        ...
        # 模拟错误
        with patch.object(self.renamer, '_extract_document_info', side_effect=Exception("测试错误")):
            result = self.renamer.generate_filename(path_plan, document_data, classification_result)
