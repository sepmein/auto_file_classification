...
def demo_file_status_management():
    ...
    try:
        ...
        index_updater._update_file_status(
            {"primary_target_path": str(low_confidence_file)}, low_confidence_result
        )
        ...
        for file_info in files_needing_review:
            print(
                f"  - {file_info['file_path']} (置信度: {file_info.get('confidence_score', 'N/A')})"
            )
...
def demo_rollback_operation():
    ...
    try:
        ...
        document_data = {"text_content": "需要回滚的文档", "embedding": [0.1, 0.2, 0.3]}
