...
class ClassificationConfig:
    """分类配置"""
    categories: List[str] = field(default_factory=lambda: ["工作", "个人", "财务", "其他"])
...
class Config:
    ...
    def load_config(self) -> None:
        ...
        try:
            ...
            if "embedding" in config_data:
                ...
                self.embedding.device = embedding_data.get("device", self.embedding.device)
                self.embedding.max_length = embedding_data.get("max_length", self.embedding.max_length)
            ...
            if "database" in config_data:
                ...
                self.database.vector_db_path = db_data.get("vector_db_path", self.database.vector_db_path)
            
            # 加载分类配置
            if "classification" in config_data:
                classification_data = config_data["classification"]
                self.classification.categories = classification_data.get("categories", self.classification.categories)
                self.classification.confidence_threshold = classification_data.get("confidence_threshold", self.classification.confidence_threshold)
                self.classification.review_threshold = classification_data.get("review_threshold", self.classification.review_threshold)
                self.classification.max_tags = classification_data.get("max_tags", self.classification.max_tags)
            
            # 加载文件配置
            if "file" in config_data:
                file_data = config_data["file"]
                self.file.source_directory = file_data.get("source_directory", self.file.source_directory)
                self.file.target_directory = file_data.get("target_directory", self.file.target_directory)
                self.file.supported_extensions = file_data.get("supported_extensions", self.file.supported_extensions)
                self.file.max_file_size = file_data.get("max_file_size", self.file.max_file_size)
            
            # 加载系统配置
            if "system" in config_data:
                system_data = config_data["system"]
                self.system.log_level = system_data.get("log_level", self.system.log_level)
                self.system.log_file = system_data.get("log_file", self.system.log_file)
                self.system.temp_directory = system_data.get("temp_directory", self.system.temp_directory)
                self.system.backup_enabled = system_data.get("backup_enabled", self.system.backup_enabled)
    ...
    def validate(self) -> bool:
        ...
        # 检查必要的目录
        if self.file.source_directory and not os.path.exists(self.file.source_directory):
