...
class DocumentParser:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 解析器优先级配置
        self.parser_priority = config.get("parser", {}).get("priority", [
            "pdf", "office", "text", "ocr"
        ])
        ...
        self.logger.info(f"文档解析器初始化完成，支持的文件类型: {self.supported_extensions}")
    ...
    def parse(self, file_path: str | Path) -> ParseResult:
        ...
        if result.success:
            ...
            self.logger.info(f"文件解析成功: {file_path}, 字数: {word_count}, 解析器: {result.parser_type}")
        else:
    ...
    def _get_possible_parsers(self, extension: str) -> List[str]:
        ...
        # 文本文件
        elif extension in ['.txt', '.md', '.markdown', '.rst', '.csv', '.json', 
                          '.xml', '.html', '.htm', '.py', '.js', '.css', 
    ...
    def _try_ocr_fallback(self, file_path: Path, original_error: str) -> ParseResult:
        ...
        try:
            # 对于PDF文件，尝试OCR解析
            if extension == '.pdf':
                if hasattr(ocr_parser, 'is_scanned_pdf') and ocr_parser.is_scanned_pdf(file_path):
    ...
    def parse_batch(self, file_paths: List[str | Path]) -> List[ParseResult]:
        ...
        self.logger.info(f"批量解析完成: 总计 {len(results)}, 成功 {success_count}, 失败 {len(results) - success_count}")
