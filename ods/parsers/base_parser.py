...
class BaseParser(ABC):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_file_size = config.get("file", {}).get("max_file_size", 100 * 1024 * 1024)  # 100MB
    ...
    def can_parse(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if file_size > self.max_file_size:
                self.logger.warning(f"文件过大，跳过解析: {file_path} ({file_size} bytes)")
                return False
        except OSError as e:
    ...
    def create_success_result(self, file_path: Path, content: ParsedContent) -> ParseResult:
        """
        创建成功结果
