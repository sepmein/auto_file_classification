...
class IndexUpdater:
    ...
    def _init_database(self):
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 创建索引
                cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_audit_file_path ON {self.audit_table}(file_path)")
                cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_audit_category ON {self.audit_table}(category)")
                cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_audit_status ON {self.audit_table}(status)")
                cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_status_category ON {self.status_table}(category)")
                cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_status_needs_review ON {self.status_table}(needs_review)")
    ...
    def update_indexes(self, move_result: Dict[str, Any], 
        ...
        try:
            ...
            results = {
                ...
                'llama_update': self._update_llama_index(move_result, document_data, classification_result) if self.enable_llama_index else {'success': False, 'reason': 'disabled'},
            ...
            # 检查整体结果
            success = all(result.get('success', False) for result in results.values() if isinstance(result, dict))
    ...
    def _update_vector_store(self, move_result: Dict[str, Any], 
                           document_data: Dict[str, Any],
    ...
    def _update_llama_index(self, move_result: Dict[str, Any], 
                          document_data: Dict[str, Any],
                          classification_result: Dict[str, Any]) -> Dict[str, Any]:
        """更新LlamaIndex知识库"""
        try:
    ...
    def _log_audit_record(self, operation_id: str, move_result: Dict[str, Any],
                         document_data: Dict[str, Any], 
                         classification_result: Dict[str, Any],
                         processing_time: float) -> Dict[str, Any]:
        """记录审计日志"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    Path(move_result.get('primary_target_path', '')).name if move_result.get('primary_target_path') else '',
                    ...
                    json.dumps(classification_result.get('rules_applied', []), ensure_ascii=False),
                    processing_time,
                    'auto',
    ...
    def _update_file_status(self, move_result: Dict[str, Any],
                          classification_result: Dict[str, Any]) -> Dict[str, Any]:
        """更新文件状态"""
        try:
            file_path = move_result.get('primary_target_path', move_result.get('original_path', ''))
            if not file_path:
    ...
    def get_audit_records(self, file_path: Optional[str] = None, 
    ...
    def get_file_status(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(f"SELECT * FROM {self.status_table} WHERE file_path = ?", (file_path,))
                row = cursor.fetchone()
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(f"SELECT COUNT(*) FROM {self.audit_table} WHERE status = 'success'")
                ...
                cursor.execute(f"SELECT COUNT(*) FROM {self.status_table} WHERE needs_review = TRUE")
                files_needing_review = cursor.fetchone()[0]
                
                # 分类统计
                cursor.execute(f"SELECT category, COUNT(*) FROM {self.status_table} GROUP BY category")
