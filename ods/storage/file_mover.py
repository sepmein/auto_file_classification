...
class FileMover:
    ...
    def move_file(
        self, path_plan: Dict[str, Any], naming_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """执行移动与链接创建的主入口
        ...
        report: Dict[str, Any] = {
            ...
            "primary_target_path": naming_result.get("new_path")
            or path_plan.get("primary_path"),
            "link_creations": [],
            "moved": False,
            "rolled_back": False,
            "errors": [],
        ...
        try:
            ...
            if link_paths:
                for link_info in link_paths:
                    created = self._create_link_for_tag(
                        target_path, link_info, operations_log
                    )
    ...
    def _move_main_file(
        self, old_path: Path, new_path: Path, operations_log: List[Dict[str, Any]]
    ):
        """移动主文件，记录操作以便回滚"""
        ...
        if self.dry_run:
            ...
            operations_log.append(
                {"op": "move_dry", "from": str(old_path), "to": str(new_path)}
            )
            return
        ...
        while final_target.exists():
            final_target = new_path.with_name(
                f"{new_path.stem}_{counter}{new_path.suffix}"
            )
            counter += 1
        ...
        operations_log.append(
            {"op": "move", "from": str(old_path), "to": str(final_target)}
        )

    def _create_link_for_tag(
        self,
        primary_target: Path,
        ...
        if self.dry_run:
            ...
            return {
                "path": str(link_file_path),
                "type": link_info.get("type", "link"),
                "ok": True,
                "dry_run": True,
            }
    ...
    def _cleanup_empty_directories(
        self, start_dir: Path, operations_log: List[Dict[str, Any]]
    ):
        """自下而上清理空目录"""
        try:
            curr = start_dir
