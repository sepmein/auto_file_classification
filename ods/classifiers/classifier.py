...
class DocumentClassifier:
    ...
    def classify_document(self, document_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 4. 如果分类成功，添加到向量数据库
            if final_result.get('primary_category') and final_result.get('primary_category') != 'Uncategorized':
                self._add_to_vector_database(document_data, final_result)
            
            self.logger.info(f"文档分类完成: {file_path} -> {final_result.get('primary_category')}")
            return final_result
    ...
    def _add_to_vector_database(self, document_data: Dict[str, Any], 
        ...
        try:
            ...
            # 生成文档ID
            doc_id = f"{int(time.time())}_{Path(document_data.get('file_path', '')).stem}"
    ...
    def batch_classify(self, documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, document in enumerate(documents):
            try:
                self.logger.info(f"处理文档 {i+1}/{len(documents)}: {document.get('file_path', '')}")
    ...
    def update_document_classification(self, doc_id: str, 
                                     new_classification: Dict[str, Any]) -> bool:
        """更新文档分类"""
        try:
            # 更新向量数据库中的元数据
            success = self.retrieval_agent.update_document(
                ...
                new_metadata={
                    ...
                    'secondary_categories': new_classification.get('secondary_categories', []),
    ...
    def search_similar_documents(self, query_embedding, top_k: int = 5) -> List[Dict[str, Any]]:
        """搜索相似文档"""
        try:
    ...
    def get_category_examples(self, category: str, top_k: int = 3) -> List[Dict[str, Any]]:
        """获取类别示例"""
        try:
    ...
    def _create_error_result(self, error_message: str, document_data: Dict[str, Any]) -> Dict[str, Any]:
        """创建错误结果"""
        return {
