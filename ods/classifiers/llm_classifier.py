    ...
    def _setup_llm_client(self):
        ...
        try:
            ...
            if self.provider == 'openai':
                ...
                self.logger.info("Ollama客户端设置成功")
                return client
...
{{
    ...
    def _post_process_classification(self, classification_result: Dict[str, Any], 
                                   document_data: Dict[str, Any],
                                   similar_docs: List[Dict[str, Any]]) -> Dict[str, Any]:
        """后处理分类结果"""
        try:
            # 确保主要类别在预定义类别中
            ...
            valid_secondary = [cat for cat in secondary_categories if cat in self.categories]
            classification_result['secondary_categories'] = valid_secondary[:self.max_tags-1]
            
            # 计算最终置信度
            confidence_score = classification_result.get('confidence_score', 0.5)
            classification_result['confidence_score'] = max(0.0, min(1.0, confidence_score))
