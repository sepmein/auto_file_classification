...
class RetrievalAgent:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 向量数据库配置
        self.vector_db_path = config.get('database', {}).get('vector_db_path', '.ods/vector_db')
    ...
    def add_document(self, doc_id: str, embedding: np.ndarray, 
                     metadata: Dict[str, Any], text_chunk: str = "") -> bool:
        """添加文档到向量数据库"""
        try:
            # 准备元数据
            doc_metadata = {
            ...
            if results['ids'] and results['ids'][0]:
                for i in range(len(results['ids'][0])):
                    doc_info = {
                        ...
                        'text_chunk': results['metadatas'][0][i].get('text_chunk', '')
                    }
            ...
            if results['ids']:
                for i in range(len(results['ids'])):
                    doc_data = {
                        'id': results['ids'][i],
                        'metadata': results['metadatas'][i],
                        'embedding': results['embeddings'][i].tolist() if results['embeddings'] else None
                    }
