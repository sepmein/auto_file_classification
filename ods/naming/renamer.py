...
class Renamer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.default_template = self.naming_config.get('default_template', '{{category}}-{{title}}-{{date}}.{{ext}}')
        ...
        self.conflict_resolution = self.naming_config.get('conflict_resolution', 'suffix')
    ...
    def generate_filename(self, path_plan: Dict[str, Any], 
        ...
        try:
            ...
            # 提取文档信息
            document_info = self._extract_document_info(document_data, classification_result)
            ...
            self.logger.info(f"文件名生成完成: {Path(original_path).name} -> {clean_filename}")
    ...
    def _extract_document_info(self, document_data: Dict[str, Any], 
                             classification_result: Dict[str, Any]) -> Dict[str, Any]:
        """提取文档信息"""
        info = {}
    ...
    def _extract_title_from_content(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if line and len(line) > 5 and len(line) < 100:
                # 检查是否像标题（不包含特殊字符，长度适中）
                if not re.search(r'[^\w\s\-_\.]', line):
    ...
    def _select_naming_template(self, category: str, document_info: Dict[str, Any]) -> str:
        """选择命名模板"""
        # 检查是否有类别特定的模板
    ...
    def _apply_naming_template(self, template: str, document_info: Dict[str, Any]) -> str:
        """应用命名模板"""
        try:
            # 使用Jinja2渲染模板
    ...
    def _simple_template_replace(self, template: str, document_info: Dict[str, Any]) -> str:
        """简单的模板替换（回退方案）"""
        result = template
    ...
    def _truncate_filename(self, filename: str) -> str:
        ...
        if len(name_parts) > 1:
            stem, ext = name_parts
            max_stem_length = self.max_filename_length - len(ext) - 1
    ...
    def _build_new_path(self, primary_path: str, new_filename: str) -> str:
        ...
        return str(primary_path_obj.parent / new_filename)

    def _check_filename_conflicts(self, new_path: str, original_path: str) -> Dict[str, Any]:
        """检查文件名冲突"""
    ...
    def _create_error_naming(self, path_plan: Dict[str, Any], error_message: str) -> Dict[str, Any]:
        """创建错误命名结果"""
        original_path = path_plan.get('original_path', '')
    ...
    def _load_naming_templates(self) -> Dict[str, str]:
        """加载命名模板"""
        templates_file = self.naming_config.get('templates_file', 'config/naming_templates.yaml')
    ...
    def _save_naming_templates(self) -> bool:
        ...
        try:
            ...
            with open(templates_file, 'w', encoding='utf-8') as f:
                yaml.dump(self.templates, f, default_flow_style=False, allow_unicode=True)
    ...
    def validate_naming_result(self, naming_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(new_filename) > self.max_filename_length:
            validation_result['warnings'].append(f"文件名长度超过限制: {len(new_filename)} > {self.max_filename_length}")
