...
class PathPlanner:
    ...
    def plan_file_path(
        ...
        try:
            ...
            # 获取分类信息
            primary_category = classification_result.get(
            ...
            # 检查是否需要人工审核
            if confidence_score < self.config.get("classification", {}).get(
    ...
    def _determine_primary_path(
        self, category: str, original_path: str, metadata: Dict[str, Any]
    ) -> str:
        """确定主存储路径"""
        # 获取文件信息
    ...
    def _plan_link_paths(
        self, tags: List[str], primary_category: str, primary_path: str
    ) -> List[Dict[str, Any]]:
        """规划链接路径（多标签情况）"""
        link_paths = []
    ...
    def _check_path_conflicts(
        self, target_path: str, original_path: str
    ) -> Dict[str, Any]:
        """检查路径冲突"""
    ...
    def _create_review_path(
        self, original_path: str, classification_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """创建需要审核的路径"""
    ...
    def _create_error_path(
        self, original_path: str, error_message: str
    ) -> Dict[str, Any]:
        """创建错误路径"""
    ...
    def _load_category_mapping(self) -> Dict[str, str]:
        """加载类别路径映射"""
        mapping_file = self.path_config.get(
    ...
    def validate_path_plan(self, path_plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 检查路径长度
        if (
            "primary_path" in path_plan
            and len(path_plan["primary_path"]) > self.max_path_length
        ):
            validation_result["warnings"].append(
                f"路径长度超过限制: {len(path_plan['primary_path'])} > {self.max_path_length}"
            )
